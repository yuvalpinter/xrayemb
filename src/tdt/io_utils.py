...
def load_model_files(model_type, model_dir, temp_dir=None):
    ...
    if not os.path.exists(temp_dir):
        ...
        model_targ = os.path.join(temp_dir, "pytorch_model.bin")
    ...
    if model_type in ['bert', 'cbert']:
        ...
        tokenizer = T5Tokenizer.from_pretrained(temp_dir)
    ...
    return tokenizer, model, temp_dir
...
def load_tdt_periphery(model_dir, checkpoint=None, temp_dir=None):
    ...
    all_chars = load_chars(cvoc_tmp_path)
    ...
    if not tf.io.gfile.exists(f'{model_dir}/config_tdt.json'):
        ...
        with open(os.path.join(temp_dir, "config_tdt.json")) as tdt_prms:
            cnf_prms = json.loads(tdt_prms.read())
            args.__dict__.update(cnf_prms)
    return all_chars, args, temp_dir


def load_chars(file_name):
    with tf.io.gfile.GFile(file_name, mode="r") as f:
...
def write_dataset(ds, file_path):
    with tf.io.gfile.GFile(file_path, mode="wb") as f:
        pickle.dump(ds, f)
